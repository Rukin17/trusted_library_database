...
# def get_library(library: models.Library, db: Session, library_id: int):
#     return db.query(library).filter(library.id == library_id).first()


# def get_libraries(library: models.Library, db: Session, skip: int = 0, limit: int = 100):
#     return db.query(library).offset(skip).limit(limit).all()


# def create_library(db: Session, library: models.Library, library_schemas: LibrarySchemas, user_id: int):
